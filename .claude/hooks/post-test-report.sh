...
# Write Python parsers to temp files — avoids heredoc-inside-$() bash parse error
_PY_COV=$(mktemp /tmp/test-cov-XXXXXX.py)
cat > "$_PY_COV" << 'PY'
import sys, json
try:
    ...
    for c in sorted(classes, key=lambda x: x.get('coveragePercent', 0) if isinstance(x, dict) else 0):
        ...
        pct       = float(c.get('coveragePercent', 0))
        covered   = int(c.get('coveredLines', 0))
        uncovered = int(c.get('uncoveredLines', 0))
        bar_w     = int(pct)
        if pct < 75:
        ...
        bar = ('<div style="background:#e2e8f0;border-radius:4px;height:10px;width:120px">'
               '<div style="background:' + color + ';width:' + str(bar_w) +
               '%;height:10px;border-radius:4px"></div></div>')
        rows.append(
            '<tr style="background:' + bg + '">'
            '<td>' + str(name) + '</td>'
            '<td style="text-align:right;font-weight:700;color:' + color + '">' + str(round(pct, 1)) + '%</td>'
            '<td>' + bar + '</td>'
            '<td style="text-align:right">' + str(covered) + '</td>'
            '<td style="text-align:right">' + str(uncovered) + '</td>'
            '</tr>'
        )
    print('\n'.join(rows))
except Exception:
...
_PY_RES=$(mktemp /tmp/test-res-XXXXXX.py)
cat > "$_PY_RES" << 'PY'
import sys, json
try:
    with open(sys.argv[1]) as f:
    ...
    print(str(total) + '|' + str(passing) + '|' + str(failing) + '|' + str(cov))
    for t in data.get('tests', []):
        if t.get('Outcome') == 'Fail':
            ...
            print('FAIL|' + cls + '.' + method + '|' + msg + '|' + stack)
except Exception:
    print('0|0|0|N/A')
PY
...
if [[ -d "$RESULTS_DIR" ]]; then
  ...
  [[ -n "$COV_FILE" ]] && COVERAGE_ROWS=$(python3 "$_PY_COV" "$COV_FILE" 2>/dev/null || echo "")

  if [[ -n "$RES_FILE" ]]; then
    SUMMARY_OUT=$(python3 "$_PY_RES" "$RES_FILE" 2>/dev/null || echo "0|0|0|N/A")
    while IFS='|' read -r a b c d; do
      if [[ "$a" =~ ^[0-9]+$ ]]; then
        TOTAL=$a; PASSED=$b; FAILED=$c; ORG_COVERAGE="${d}%"
      elif [[ "$a" == "FAIL" ]]; then
...
rm -f "$_PY_COV" "$_PY_RES"

# Badge colour
if [[ "$FAILED" -gt 0 ]]; then
  BADGE_COLOR="#dc2626"; BADGE_TEXT="FAILING"
  ...
  BADGE_COLOR="#d97706"; BADGE_TEXT="WARNINGS"
fi
...
$([ -n "$FAILED_ROWS" ] && printf '<section><h2>🔴 Failed Tests</h2><table><tr><th>Test Method</th><th>Error</th><th>Stack</th></tr>%s</table></section>' "$FAILED_ROWS")
<section>
  ...
  <table>
    ...
    ${COVERAGE_ROWS:-<tr><td colspan="5" style="text-align:center;padding:24px;color:#94a3b8">No coverage data in /tmp/apex-test-results/</td></tr>}
  </table>
</section>
